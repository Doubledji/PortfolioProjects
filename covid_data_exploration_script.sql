...
WHERE 
  continent IS NOT NULL 
ORDER BY 
  1,2 -- in this case location and date become the 1 and 2 columns
...
SELECT 
  ...
  population, 
  total_cases, 
  (NULLIF(total_cases, 0)/NULLIF(population, 0))*100 AS percent_population_infected
FROM 
  covid_deaths
--WHERE location = 'Portugal'
ORDER BY 
...
SELECT 
  ...
  MAX(NULLIF(total_cases, 0)/NULLIF(population, 0))*100 AS percent_population_infected
FROM 
  covid_deaths
--WHERE location = 'Portugal'
GROUP BY 
...
FROM 
  covid_deaths
WHERE 
  continent IS NOT NULL -- null data was grouping entire continents and we want the country information is this query
...
DROP TABLE IF EXISTS percent_population_vaccinated -- Important to avoid wasting server resources and errors
