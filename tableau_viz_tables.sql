...
--WHERE location = 'Portugal'
WHERE 
 location NOT IN ('World', 'Europe','European Union', 'International', 'Upper middle income', 'High income', 'Lower middle income', 'Low income', 'Africa', 'South America', 'North America', 'Oceania')
GROUP BY 
  location, 
  population, 
  date
...
SELECT 
  ...
  SUM(new_cases) AS total_cases, 
  SUM(new_deaths) AS total_deaths, 
  SUM(new_deaths)/SUM(NULLIF(new_cases, 0))*100 AS death_percentage
FROM 
  covid_deaths
--WHERE location = 'Portugal'
WHERE 
